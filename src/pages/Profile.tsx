...
import { showSuccessfully } from './Dashboard/Notification/Successfully';
import { showError } from './Dashboard/Notification/Error';
import { showWarning } from './Dashboard/Notification/Warning';
...
const Profile = () => {
  ...
  useEffect(() => {
    ...
    const storedUserImage = sessionStorage.getItem('admin_photo');
    const storedUserBackground = sessionStorage.getItem('admin_background');
    if (storedUsername) {
      setUsername(storedUsername);
    }
    
    if (storedUserImage) {
      setUserImage(`${IMG_URL}/image/${storedUserImage}`);
    } else {
      setUserImage(`${IMG_URL}/image/defaultuser.jpg`);
    }

    if (storedUserBackground) {
      setUserBackground(`${IMG_URL}/image/${storedUserBackground}`);
    } else {
      setUserBackground(CoverOne);
    }
  }, []);
  ...
  const handleUpdateProfileImage = async () => {
    if (selectedProfileImage) {
      ...
      try {
        await apiUser.updateUserImage(formData);
        showSuccessfully('Bạn đã thay đổi ảnh đại diện thành công');
        setUserImage(URL.createObjectURL(selectedProfileImage));
        setPreviewProfileImage(null);
        setSelectedProfileImage(null);
      } catch (error) {
        console.error('Error updating profile image:', error);
        showError('Có lỗi xảy ra khi cập nhật ảnh đại diện');
      }
    } else {
      showWarning('Vui lòng chọn ảnh đại diện trước khi cập nhật');
    }
  };
  ...
  const handleUpdateBackgroundImage = async () => {
    if (selectedBackgroundImage) {
      ...
      try {
        await apiUser.updateUserBackground(formData);
        showSuccessfully('Bạn đã thay hình nền thành công');
        setUserBackground(URL.createObjectURL(selectedBackgroundImage));
        setPreviewBackgroundImage(null);
        setSelectedBackgroundImage(null);
      } catch (error) {
        console.error('Error updating background image:', error);
        showError('Có lỗi xảy ra khi cập nhật hình nền');
      }
    } else {
      showWarning('Vui lòng chọn hình nền trước khi cập nhật');
    }
  };
  ...
  const handleCancelBackgroundImageChange = () => {
    setPreviewBackgroundImage(null);
    setSelectedBackgroundImage(null);
  };
    ...
    <>
      ...
      <div className="overflow-hidden rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark">
        <div className="relative z-20 h-35 md:h-65">
          <img
            src={previewBackgroundImage || userBackground}
            alt="profile cover"
          ...
          <div className="absolute bottom-1 right-1 z-10 xsm:bottom-4 xsm:right-4">
            ...
            >
              <input type="file" name="cover" id="cover" className="sr-only" onChange={handleBackgroundImageChange} />
              <span>
                <svg
          ...
          {previewBackgroundImage && (
            <div className="absolute bottom-1 left-1 z-10 xsm:bottom-4 xsm:left-4 flex space-x-2">
              <button
              ...
              <button
                onClick={handleCancelBackgroundImageChange}
                className="bg-primary text-white py-1 px-2 rounded text-sm"
              >
                Hủy
              </button>
        ...
        <div className="px-4 pb-6 text-center lg:pb-8 xl:pb-11.5">
          <div className="relative z-30 mx-auto -mt-22 h-30 w-30 rounded-full bg-white/20 p-1 backdrop-blur sm:h-44 sm:w-44 sm:p-3">
            <div className="relative drop-shadow-2 w-full h-full">
              <img src={previewProfileImage || userImage} alt="profile" className="rounded-full w-full h-full object-cover" />
              <label
                htmlFor="profile"
          ...
          {previewProfileImage && (
            <div className="mt-2 flex justify-center space-x-2">
              ...
              <button
                ...
                className="bg-gray-500 text-white py-1 px-2 rounded text-sm"
              >
                Cancel
              </button>
            </div>
