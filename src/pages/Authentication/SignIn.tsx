...
const SignIn: React.FC = () => {
  ...
  useEffect(() => {
    const savedEmail = Cookies.get('admin_email');
    const savedPassword = Cookies.get('admin_password');
    const savedRememberMe = Cookies.get('admin_rememberMe') === 'true';
    const savedRoleId = Cookies.get('admin_role_id');
    if (savedRememberMe && savedEmail && savedPassword && savedRoleId) {
      setEmail(savedEmail);
  ...
  const handleSignIn = async (e: React.FormEvent) => {
    ...
    try {
      const result = await apiUser.loginUser(data);
      console.log("result: ", result);
      if (result.status === 200) {
        if (result.data.user.role_id === 1 || result.data.user.role_id === 2) {
          sessionStorage.setItem('admin_email', result.data.user.email);
          sessionStorage.setItem('admin_username', result.data.user.name);
          sessionStorage.setItem('admin_password', password);
          sessionStorage.setItem('admin_user_id', result.data.user.id);
          sessionStorage.setItem('admin_role_id', result.data.user.role_id.toString());
          sessionStorage.setItem('admin_rememberMe', rememberMe.toString());
          sessionStorage.setItem('admin_background', result.data.user.background || '');
          sessionStorage.setItem('admin_photo', result.data.user.photo || '');
          localStorage.setItem('admin_token', result.data.token);
          
          if (rememberMe) {
            const cookieOptions = { expires: 7 }; // Set cookies to expire in 7 days
            Cookies.set('admin_email', result.data.user.email, cookieOptions);
            Cookies.set('admin_password', password, cookieOptions);
            Cookies.set('admin_role_id', result.data.user.role_id.toString(), cookieOptions);
            Cookies.set('admin_rememberMe', 'true', cookieOptions);
          } else {
            // If "Remember Me" is not checked, remove any existing cookies
            Cookies.remove('admin_email');
            Cookies.remove('admin_password');
            Cookies.remove('admin_role_id');
            Cookies.remove('admin_rememberMe');
          }
          
          // Set session cookie for admin
          Cookies.set('admin_session', result.data.sessionID, { 
            expires: 1/24, // 1 hour
            path: '/admin'
          });
          ...
          // Redirect to home page and force a reload
  ...
  return (
    <>
      <div className={`flex items-center justify-center min-h-screen w-full ${isDarkMode ? 'dark:bg-boxdark' : 'bg-white'}`}>
        <button
        ...
        <div className="flex flex-wrap items-center">
          <div className="hidden w-full xl:block xl:w-1/2">
            <div className="py-17.5 px-26 text-center">
              <p className="2xl:px-20">Đây là trang quản lí nơi bạn có thể làm trời làm đất</p>
          ...
          <div className="w-full border-stroke dark:border-strokedark xl:w-1/2 xl:border-l-2">
            <div className="w-full p-4 sm:p-12.5 xl:p-17.5">
              <h2 className="mb-9 text-2xl font-bold text-black dark:text-white sm:text-title-xl2">
                Chào mừng bạn đến với trang quản lí 
              </h2>
                  ...
                  <div className="relative">
                    ...
                    {email && (
                      ...
                      >
                        ...
                        >
                          <path
                            d="M16.293 17.707L11 12.414L5.707 17.707C5.51839 17.8945 5.26579 17.9998 5.00248 17.9998C4.73917 17.9998 4.48657 17.8945 4.29796 17.707C4.10936 17.5184 4.00404 17.2658 4.00404 17.0025C4.00404 16.7392 4.10936 16.4866 4.29796 16.298L9.586 11L4.29296 5.707C4.10544 5.51839 4.00112 5.26579 4.00112 5.00248C4.00112 4.73917 4.10544 4.48657 4.29296 4.29796C4.48048 4.10936 4.73308 4.00404 4.99639 4.00404C5.2597 4.00404 5.5123 4.10936 5.69982 4.29796L11 9.586L16.293 4.293C16.4816 4.10448 16.7342 4.00016 16.9975 4.00016C17.2608 4.00016 17.5134 4.10448 17.702 4.293C17.8906 4.48152 17.9959 4.73412 17.9959 4.99743C17.9959 5.26074 17.8906 5.51334 17.702 5.70186L12.414 11L17.707 16.293C17.8946 16.4816 17.9999 16.7342 17.9999 16.9975C17.9999 17.2608 17.8946 17.5134 17.707 17.702C17.5184 17.8906 17.2658 17.9959 16.9975 17.9959C16.7292 17.9959 16.4766 17.8906 16.288 17.702L16.293 17.707Z"
                            fill=""
                          />
                        </svg>
                      </button>
                  ...
                  <div className="relative">
                    ...
                    >
                      {showPassword ? (
                        ...
                        >
                          <path
                            d="M2.75 11.75C2.75 11.75 5.5 6.5 11 6.5C16.5 6.5 19.25 11.75 19.25 11.75M19.25 11.75C19.25 11.75 16.5 17 11 17C5.5 17 2.75 11.75 2.75 11.75M19.25 11.75H2.75"
                            stroke="currentColor"
                            strokeWidth="1.5"
                            strokeLinecap="round"
                            strokeLinejoin="round"
                          />
                          <path
                            d="M11 14.25C12.2426 14.25 13.25 13.2426 13.25 12C13.25 10.7574 12.2426 9.75 11 9.75C9.75736 9.75 8.75 10.7574 8.75 12C8.75 13.2426 9.75736 14.25 11 14.25Z"
                            stroke="currentColor"
                            strokeWidth="1.5"
                            strokeLinecap="round"
                            strokeLinejoin="round"
                        ...
                        >
                          <path
                            d="M11 5C15.41 5 19.18 7.64 20.45 11.55C20.55 11.84 20.55 12.16 20.45 12.45C19.18 16.36 15.41 19 11 19C6.59 19 2.82 16.36 1.55 12.45C1.45 12.16 1.45 11.84 1.55 11.55C2.82 7.64 6.59 5 11 5ZM11 17C14.31 17 17.23 15.06 18.45 12C17.23 8.94 14.31 7 11 7C7.69 7 4.77 8.94 3.55 12C4.77 15.06 7.69 17 11 17ZM11 15C9.34 15 8 13.66 8 12C8 10.34 9.34 9 11 9C12.66 9 14 10.34 14 12C14 13.66 12.66 15 11 15Z"
                            fill=""
                          />
                        </svg>
                      )}
